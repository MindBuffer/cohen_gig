...
fn hsv(h: f32, s: f32, v: f32) -> Vector3 {
    ...
    let f1 = vec3(f.x.fract(), f.y.fract(), f.z.fract()) * vec3(6.0,6.0,6.0) - vec3(3.0,3.0,3.0);
    let f2 = vec3(f1.x.abs(), f1.y.abs(), f1.z.abs()) - vec3(1.0,1.0,1.0);
    let f3 = vec3(clamp(f2.x, 0.0, 1.0), clamp(f2.y, 0.0, 1.0), clamp(f2.z, 0.0, 1.0));
    vec3( mix(1.0, f3.x, s) * v,mix(1.0, f3.y, s) * v,mix(1.0, f3.z, s) * v)
}

fn circle(uv: Vector2, r: f32, thickness: f32) -> f32 {
...
pub fn shader(p: Vector3, uniforms: &Uniforms) -> LinSrgb {
    ...
    let p1 = vec2(fmod(uv.x,rep.x)-rep.x*0.5, fmod(uv.y,rep.y)-rep.y*0.5);
    let p2 = vec2(fmod(uv.x+2.0,rep.x)-rep.x*0.5, fmod(uv.y,rep.y)-rep.y*0.5);
    let p3 = vec2(fmod(uv.x+1.0,rep.x)-rep.x*0.5, fmod(uv.y+r3,rep.y)-rep.y*0.5);
    let p4 = vec2(fmod(uv.x+3.0,rep.x)-rep.x*0.5, fmod(uv.y+r3,rep.y)-rep.y*0.5);
    let p5 = vec2(fmod(uv.x+0.0,rep.x)-rep.x*0.5, fmod(uv.y+r3*2.0,rep.y)-rep.y*0.5);
    let p6 = vec2(fmod(uv.x+2.0,rep.x)-rep.x*0.5, fmod(uv.y+r3*2.0,rep.y)-rep.y*0.5);
    let p7 = vec2(fmod(uv.x+1.0,rep.x)-rep.x*0.5, fmod(uv.y+r3*2.0,rep.y)-rep.y*0.5);
    let p8 = vec2(fmod(uv.x+3.0,rep.x)-rep.x*0.5, fmod(uv.y+r3*2.0,rep.y)-rep.y*0.5);
