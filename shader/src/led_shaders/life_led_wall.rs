...
pub fn shader(v: Vertex, uniforms: &Uniforms) -> LinSrgb {
    ...
    let mut brightness = vec3(
        ...
        (rand(vec2(uv.x.floor(), uv.y.floor())) + t / (1.0 + (params.colour_offset * 2.0)) * s)
            .fract(),
        (rand(vec2(uv.x.floor(), uv.y.floor())) + t / (1.0 + (params.colour_offset * 3.0)) * s)
            .fract(),
    );
