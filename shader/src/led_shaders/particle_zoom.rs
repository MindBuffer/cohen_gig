...
fn cell(coord: Vector2, p: &Params) -> f32 {
...
pub fn shader(p: Vector3, uniforms: &Uniforms) -> LinSrgb {
    let params = Params {
        ...
        shape: 0.35,
        tau: 10.0,
        y_offset: (uniforms.time * 0.1).sin(), // -1. - 1
    };
    ...
    let mut uv = vec2(x,y) / uniforms.resolution - vec2(0.0,params.y_offset * 0.005); 
    uv *= uniforms.resolution.x / uniforms.resolution.y;

    let a = (atan(uv.x,uv.y) / params.tau).fract();
