...
pub fn shader(p: Vector3, uniforms: &Uniforms) -> LinSrgb {
    ...
    let y = map_range(p.y, 0.3, 1.05, 0.0, 1.00);
    ...
    xz = multiply_mat2_with_vec2(rotate_2d(-t / params.rot1), xz);
    let mut xy = vec2(xz.x, st.y);
    xy = multiply_mat2_with_vec2(rotate_2d(t / params.rot2), xy);
    st = vec3(xy.x, xy.y, xz.y);
    let o = vec2((t/params.rot1).cos(),(t/params.rot2).sin());
    let mut col = vec![0.0; 3];
    for i in 0..3 {
        t += 0.3 * spiral(o+vec2(st.z,st.y),params.g1,16.0+128.0*o.x-o.y,-t/100.0,1.0)
            * spiral(o-vec2(st.x,st.z),params.g2,16.0+64.0*o.x-o.y,t/100.0,-1.0);
        col[i] = (params.colours*t-length(vec3(st.x,st.y,0.0))*10.0*sinp(t)).sin();
    }
