...
fn calc(tx: Vector2, t: f32, params: &shader_shared::GilmoreAcid, uniforms: &Uniforms) -> f32 {
    let angle = PI * nsin(t * 0.1) + PI / 6.0;
    let len = 1.0 - length(vec3(tx.x,tx.y,0.)) * nsin(t);
    let mut value = 0.0;
    let hex = coord_to_hex(tx, map_range(params.grid_size,0.0,1.0,0.0,200.0) * nsin(t * params.zoom_amount), angle * params.rotation_amount);
    for i in 0..3 {
