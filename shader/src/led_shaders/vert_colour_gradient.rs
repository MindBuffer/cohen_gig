...
fn hue(colour: Vec3, shift: f32) -> Vec3 {
    let k_rgb_to_yprime: Vec3 = vec3(0.299, 0.587, 0.114);
    let k_rgb_to_i: Vec3 = vec3(0.596, -0.275, -0.321);
    let k_rgb_to_q: Vec3 = vec3(0.212, -0.523, 0.311);

    let k_yiq_to_r: Vec3 = vec3(1.0, 0.956, 0.621);
    let k_yiq_to_g: Vec3 = vec3(1.0, -0.272, -0.647);
    let k_yiq_to_b: Vec3 = vec3(1.0, -1.107, 1.704);
...
pub fn shader(v: Vertex, uniforms: &Uniforms) -> LinSrgb {
    ...
    f = mix(
        f,
        f.max(((fmod(uv.y + uv.x * 1.5, 1.0) - 0.5).abs() - 0.4) * ba),
        params.diag_amp,
    );
    f = mix(
        f,
